        ...
        if t == itemLoc:
            for drops in itemDrop:
                ...
                if User["Current Biome"] in spot[1]:
                    ...
                    n = (spotList[0]) % leMap
                    e = spotList[1] % wiMap
                    if "Wall" in drops: Call.biMap.setStuffPos((leMap-1)-n,e,"Ⅱ")#Add mechanic to destory walls. Make it so instead
                    ...
                    if spotList == space:
                        if "Wall" in drops: Movement.pre = "Ⅱ" 
...
def dropItem(said):
    ...
    leMap = Call.biMap.getLength()
    wiMap = Call.biMap.getWidth()
    if "Wall" in it: Movement.pre = "Ⅱ" #Add mechanic to destory walls (░♀▒▓█▄▀▐▌)
